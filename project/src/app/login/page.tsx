...
type RoleType = 'siswa' | 'guru';

export default function LoginPage() {
  const router = useRouter();
  const [role, setRole] = useState<RoleType>('siswa');
  const [identifier, setIdentifier] = useState('20241087');
  ...
  const roleConfig = {
    siswa: {
      title: 'Siswa',
      subtitle: 'Portal Pembelajaran & Presensi Face ID',
      inputLabel: 'NISN (Nomor Induk Siswa Nasional)',
      placeholder: 'Masukkan NISN (Hanya Angka, misal: 20241087)',
      defaultVal: '20241087',
      defaultPass: 'siswa123',
      buttonText: 'Masuk sebagai Siswa',
      destination: '/siswa',
      icon: GraduationCap,
      hint: 'Wajib menggunakan NISN valid. Penggunaan email dilarang.',
    },
    guru: {
      title: 'Guru',
      subtitle: 'Portal Pendidik & Rekap Presensi',
      inputLabel: 'NIP (Nomor Induk Pegawai)',
      placeholder: 'Masukkan NIP (Contoh: 19850412 201101 2 003)',
      defaultVal: '19850412 201101 2 003',
      defaultPass: 'guru123',
      buttonText: 'Masuk sebagai Guru',
      destination: '/guru',
      icon: School,
      hint: 'Wajib menggunakan NIP resmi kepegawaian. Penggunaan email dilarang.',
    },
  };

  const handleRoleChange = (newRole: RoleType) => {
    setRole(newRole);
    setIdentifier(roleConfig[newRole].defaultVal);
    setPassword(roleConfig[newRole].defaultPass);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data?.success) {
        setErrorMessage(data?.error?.message || data?.message || 'Kredensial tidak valid.');
        setIsLoading(false);
        return;
      }

      router.push(roleConfig[role].destination);
    } catch {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[#f8fafc]">
      {/* LEFT COLUMN: School Branding Hero */}
      <div className="lg:w-[48%] bg-gradient-to-br from-[#1864da] via-[#1552b5] to-[#0f3d8a] text-white p-8 lg:p-16 flex flex-col justify-between relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            ...
            {/* School Tagline on Left Branding */}
        ...
        {/* Bottom Version & School Tagline Footer */}
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Pilih portal Siswa atau Guru untuk mengakses akun Anda
            </p>
          </div>

          {/* Card Container */}
          <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/60 border border-slate-200/80 overflow-hidden">
            {/* Role Header Pill / Switcher matching design */}
            <div className="bg-[#12273e] text-white p-5 text-center flex flex-col items-center justify-center relative">
              <div className="w-11 h-11 rounded-full bg-white/10 flex items-center justify-center mb-2 border border-white/20">
                <CurrentRoleIcon className="w-5 h-5 text-blue-300" />
              </div>
              <span className="text-base font-bold tracking-wide">{roleConfig[role].title}</span>
              <span className="text-[11px] text-slate-400 mt-0.5">{roleConfig[role].subtitle}</span>

              {/* Role Selection Switcher Tabs (Hanya Siswa & Guru - Admin Disembunyikan) */}
              <div className="grid grid-cols-2 gap-1 mt-4 bg-slate-800/80 p-1 rounded-full border border-slate-700 w-64">
                {(['siswa', 'guru'] as RoleType[]).map((r) => {
                  const TabIcon = r === 'siswa' ? GraduationCap : School;
                  return (
                    <button
                      key={r}
                      type="button"
                      onClick={() => handleRoleChange(r)}
                      className={`py-1.5 px-3 rounded-full text-xs font-semibold capitalize transition-all flex items-center justify-center gap-1.5 ${
                        role === r
                    ...
                    >
                      ...
                      <span>{r === 'siswa' ? 'Siswa (NISN)' : 'Guru (NIP)'}</span>
                    </button>
                  );
                })}
              </div>
            ...
            {errorMessage && (
              <div className="mx-6 mt-6 p-3.5 bg-rose-50 border border-rose-200 rounded-xl flex items-start gap-2.5 text-xs text-rose-700">
                ...
                <div>
                  <span className="font-semibold block">Validasi Keamanan Gagal:</span>
            ...
            <form onSubmit={handleLogin} className="p-7 space-y-5">
              {/* Identifier Input (NISN atau NIP) */}
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-semibold text-slate-700">
                    {roleConfig[role].inputLabel}
                  </label>
                  <span className="text-[10px] text-slate-400 font-medium">
                    {role === 'siswa' ? 'Hanya Angka' : 'Format Resmi'}
                  </span>
                </div>
                <div className="relative">
                  ...
                  <input
                    type="text"
                    required
                    inputMode={role === 'siswa' ? 'numeric' : 'text'}
                    value={identifier}
                    onChange={(e) => {
                    ...
                    className="w-full pl-10 pr-4 py-2.5 text-sm text-slate-800 bg-white rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-slate-400 font-medium tracking-wide"
                  />
                </div>
              ...
              <div className="flex items-center justify-between text-xs pt-1">
                ...
                <button
                  type="button"
                  onClick={() => alert('Untuk pemulihan akun NISN/NIP, silakan hubungi bagian IT Akademik Gedung C.')}
          ...
          <div className="mt-6 flex items-center justify-center gap-2 text-[11px] text-slate-400 text-center">
            ...
            <span>
              Sistem Keamanan Terenkripsi · Akses portal dibatasi sesuai hak akses resmi SMKN 11 Jakarta
            </span>
          </div>
        </div>
      </div>
